...
####################################################################################################
def RooDraw(opts,can,C,x,rh,model,qPDF,wPDF,tPDF,archive,chi2_val,n_param,title):
	can.cd(C+1)
#	can.cd(0)
	can.SetTitle(title)
	can.SetFillColor(0)
...
# part 2
	...
	pave = TPaveText(0.75,0.7,0.9,0.85,"NDC")
...
## Containers
		...
		wPDF,tPDF,qPDF,sPDF = {},{},{},{}
		yW, yZ,yT,yQ     =  {} ,{},{},{}
		can = TCanvas("canD","%s"%opts.function,600,600)
		can.Divide(2,2)
## Category loop
...
#				brn_params[nbp].add(brn[nb])
  ...
  ### Model
			if C==0:
				[qcd_pdf[N],params[N]] = generate_pdf(x, pdf_name=opts.function,gcs=gcs) 
			...
			#	qcd_pdf_aux[N]  = RooBernstein("qcd_model_aux_%s_CAT%d"%(''.join(opts.TF),Cp),"qcd_model_aux_%s_CAT%d"%(''.join(opts.TF),Cp),x,brn_params["CAT%d"%(sum(SC.ncats[0:iS]))])
				...
				for ib in range(n_param): 
					gcs[ib].setConstant(kTRUE)
					gcs[ib].Print()
				#	gcs_aux[ib].setConstant(kTRUE)
				#	gcs_aux[ib].Print()
					params_aux.add(gcs[ib])
				if (opts.function.find('Pol')==-1) : 
					params_aux.add(x)
					formula=generate_formula(pdf_name=opts.function,x_name=x_name)
...
#			yS    = RooRealVar("yield_signal_mass%d_CAT%d"%(MASS,Cp),"yield_signal_mass%d_CAT%d"%(MASS,Cp),ySVBF.getValV()+ySGF.getValV(),0.,400.)
  ...
  ### Combined model
	  	#	model[N] = RooAddPdf("bkg_model_CAT%d"%(Cp),"bkg_model_CAT%d"%(Cp),RooArgList(wPDF[N],tPDF[N],qcd_pdf[N]),RooArgList(yW[N],yT[N],yQ[N]))
	  		model[N] = RooAddPdf("bkg_model_%s_CAT%d"%(opts.TF[iS],Cp),"bkg_model_%s_CAT%d"%(opts.TF[iS],Cp),RooArgList(wPDF[N],tPDF[N],qcd_pdf[N]),RooArgList(yW[N],yT[N],yQ[N]))
  ...
  ### Draw
  			RooDraw(opts,can,C,x,rh[N],model[N],qcd_pdf[N],wPDF[N],tPDF[N],archive,chi2_val,n_param,opts.function)
			can.cd(C+1)
		#	can.cd(0)
			pCMS1.Draw()
			pCMS2.Draw()
  ...
  ### Reset constant settings
	...
	#				brn[nb].setConstant(kFALSE if opts.forfit else kTRUE)  ## false for final fit, true if fixed
			for ib in range(n_param): 
				if C==0:
					gcs[ib].setConstant(kFALSE if opts.forfit else kTRUE)  ## false for final fit, true if fixed)
			if not C==0:
				ntf = "trans_%s_CAT%d"%(opts.TF[iS],Cp)
