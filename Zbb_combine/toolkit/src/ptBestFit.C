...
void ptBestFit(float BIN_SIZE=5.0,bool BLIND=false,TString NAME="",TString saveNAME="")
{
 // gROOT->ProcessLine(".x ../../common/styleCMSTDR.C");
  ...
  float XMIN = 60;
  float XMAX = 170; 
  TString path1="/afs/cern.ch/work/n/nchernya/VBFHbb_2016/Zbb_combine/toolkit/src/v25_cat4_test/";
//  TString path2="/afs/cern.ch/work/n/nchernya/VBFHbb_2016/Zbb_combine/toolkit/src/run_cat4_unblind/";
  TString path2="/afs/cern.ch/work/n/nchernya/VBFHbb_2016/Zbb_combine/toolkit/src/run_cat4_unblind_final/";
  TFile *f1 = TFile::Open(path1+"datacards/datacard_zbb.root");
//  TFile *f2 = TFile::Open(path2+"/mlfit"+NAME+".root");
  TFile *f2 = TFile::Open(path2+"/mlfitANobs.root");
  TFile *f3 = TFile::Open(path1+"root/shapes_workspace.root");
  TFile *f4 = TFile::Open(path1+"root/data_shapes_workspace.root");
...
//  RooRealVar *rFit     = (RooRealVar*)((res_s->floatParsFinal()).find("r"))[0];
...
//  w->allVars()->Print();
  ...
  for(RooAbsData *ds = (RooAbsData*)next();ds != 0; ds = (RooAbsData*)next()) {
    ...
    RooRealVar *yield_vbf = (RooRealVar*)wSig->var("yield_ZJets_"+TString(ds->GetName()));
  ...
  //  RooRealVar *qcd_norm_final = (res_s->floatParsFinal()).find("CMS_vbfbb_qcd_norm_"+ds_name);
    ...
    if (BLIND) {
      ...
      pdfi->plotOn(frame1,Components("shapeBkg_qcd_"+ds_name+",shapeBkg_top_"+ds_name+",shapeBkg_wjets_"+ds_name),VisualizeError(*res_s,1,kTRUE),FillColor(0),MoveToBack());
      pdfi->plotOn(frame1,Components("shapeBkg_qcd_"+ds_name+",shapeBkg_top_"+ds_name+",shapeBkg_wjets_"+ds_name),LineWidth(2),LineStyle(3));
     ...
     // pdfi->plotOn(frame1,Components("shapeBkg_qcd_"+ds_name),LineWidth(2),LineStyle(5),LineColor(kGreen+2));
      pdfi->plotOn(frame1,Components("shapeBkg_qcd_"+ds_name+",shapeBkg_top_"+ds_name+",shapeBkg_wjets_"+ds_name),LineWidth(2),LineStyle(2),LineColor(kBlack)); 
      cout<<"chi2/ndof (bkg) = "<<frame1->chiSquare()<<endl;
		chi2sumB += frame1->chiSquare()*ds_coarse.numEntries();
		pdfi->plotOn(frame1,Components("shapeBkg_qcd_"+ds_name+",shapeBkg_top_"+ds_name+",shapeBkg_wjets_"+ds_name),LineWidth(2),LineStyle(2),LineColor(kBlack),VisualizeError(*res_s,1,kTRUE),FillColor(0),MoveToBack()); 
      hresid = frame1->residHist();
      frame2->addPlotable(hresid,"pE1");
    
      float yield_sig = rFit->getValV()*(yield_vbf->getValV());
      RooAbsPdf *signal_pdf = (RooAbsPdf*)w->pdf("shapeSig_Zbb_"+ds_name);
...
//	 }
    ...
    TCanvas* canFit = new TCanvas("Zbb_fit_"+ds_name,"Zbb_fit_"+ds_name,900,750);
    ...
    frame1->Draw();
	 TGaxis::SetExponentOffset(-0.05,0,"xy");
    gPad->Update();
	...
	//list->Print();
    if (BLIND) {
      errorBand = (RooCurve*)list->FindObject("pdf_bin"+ds_name+"_Norm[mbbReg_"+ds_name+"]_errorband_Comp[shapeBkg_qcd_"+ds_name+",shapeBkg_top_"+ds_name+",shapeBkg_wjets_"+ds_name+"]");
      gFit = (RooCurve*)list->FindObject("pdf_bin"+ds_name+"_Norm[mbbReg_"+ds_name+"]"+"_Comp[shapeBkg_qcd_"+ds_name+",shapeBkg_top_"+ds_name+",shapeBkg_wjets_"+ds_name+"]");
    }
    else {
    ...
    gBkgFit = (RooCurve*)list->FindObject("pdf_bin"+ds_name+"_Norm[mbbReg_"+ds_name+"]"+"_Comp[shapeBkg_qcd_"+ds_name+",shapeBkg_top_"+ds_name+",shapeBkg_wjets_"+ds_name+"]");
...
//    hUnc2H->GetYaxis()->SetLabelSize(0.04);
    ...
    //list1->Print();
    RooCurve *gSigFit = (RooCurve*)list1->FindObject("shapeSig_Zbb_"+ds_name+"_Norm[mbbReg_"+ds_name+"]");
...
//	 canFit->SaveAs(TString::Format("%s/plot/fits/Fit_mH%s_%s_%s.eps",path.Data(),MASS.Data(),ds_name.Data()).Data(),NAME.Data());
	...
	//	if (!(ds_name.CompareTo(saveNAME))){
	 canFit->SaveAs(TString::Format("%s/plot/fits/FitZbb_%s_%sprelimPAS_common.pdf",path.Data(),ds_name.Data(),NAME.Data()).Data());
	 canFit->SaveAs(TString::Format("%s/plot/fits/FitZbb_%s_%sprelimPAS_common.png",path.Data(),ds_name.Data(),NAME.Data()).Data());
	 canFit->SaveAs(TString::Format("%s/plot/fits/FitZbb_%s_%sprelimPAS_common.eps",path.Data(),ds_name.Data(),NAME.Data()).Data());
//	}
