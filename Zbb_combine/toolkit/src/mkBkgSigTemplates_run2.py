...
# Setup
	...
	fnames = json.loads(filecontent("%s/vbfZbb_samples_2016_all.json"%basepath))
...
### File loop
  ...
  #### Draw mass
				if (group=='WJets'):	
					if Cp==0 : NBINS=11
					if Cp==1 : NBINS=15   ##########didnt have that one before (in cat4_test)
					if Cp==2 : NBINS=11
					if Cp==3 : NBINS=11
				elif group=='ZJets' : NBINS=25
				else : NBINS=20
...
#
 ...
 ### Group yields
			...
			for hi,hY in enumerate(hMbbYield.itervalues()):
				...
				elif 'ZJets' in nh: hZY = hY.Clone("ZYield")
...
#### Start of RooFit part 
  ...
  ## W Fit
			...
			mW              = RooRealVar("W_mean_CAT%d_13TeV"%Cp,"W_mean_CAT%d_13TeV"%Cp,80,70,100)
			sW              = RooRealVar("W_sigma_CAT%d_13TeV"%Cp,"W_sigma_CAT%d_13TeV"%Cp,12,5,20)
  ...
  ### Fit W
			...
			resW.Print()
	#		intW = modelW.createIntegral(x, NormSet(x))
	#		print "W model integral = ",intW 
  ### Draw W
  ...
  ## Z Fit
			...
			mZ              = RooRealVar("Z_mean_CAT%d_13TeV"%Cp,"Z_mean_CAT%d_13TeV"%Cp,91,70,110)
			sZ              = RooRealVar("Z_sigma_CAT%d_13TeV"%Cp,"Z_sigma_CAT%d_13TeV"%Cp,12,9,20)
...
#### Set Constants
			for o in [mW,sW,aW,nW,Wb0,Wb1,Wb2,fWsig,mT,sT,mZ,sZ,aZ,nZ,Zb0,Zb1,Zb2,fZsig]: 
				o.setConstant(kTRUE)
			print "Z fraction = ", fZsig.getVal() 
			print "W fraction = ", fWsig.getVal() 
#### Import objects
			for o in [modelZ,modelW,modelT,YZ,YW,YT,YTT,YST]:
...
#
		...
		makeDirs("%s/plot/Templates/"%opts.workdir)
		canW.SaveAs("%s/plot/Templates/%s.pdf"%(opts.workdir,canW.GetName()))
		canW.SaveAs("%s/plot/Templates/%s.png"%(opts.workdir,canW.GetName()))
		canT.SaveAs("%s/plot/Templates/%s.pdf"%(opts.workdir,canT.GetName()))
		canT.SaveAs("%s/plot/Templates/%s.png"%(opts.workdir,canT.GetName()))
		canZ.SaveAs("%s/plot/Templates/%s.pdf"%(opts.workdir,canZ.GetName()))
		canZ.SaveAs("%s/plot/Templates/%s.png"%(opts.workdir,canZ.GetName()))
...
#
	...
	makeDirs("%s/root/"%opts.workdir)
	w.writeToFile("%s/root/shapes_workspace%s.root"%(opts.workdir,"" if not opts.long else longtag))
