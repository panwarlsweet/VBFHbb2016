...
#
	...
	mg1.add_option('--SELCATs',help=colours[5]+'Selection/Category setup: "Single;SingleB;0.28#0.72#0.87#0.93#1.,Double;DoubleB;0.36#0.76#0.89#1.,...".'+colours[0],default='Single;SingleB;0.28#0.72#0.87#0.93#1.,Double;DoubleB;0.36#0.76#0.89#1.',type='str',action='callback',callback=SELsetup,dest='SC')# final
	mp.add_option_group(mg1)
#
	...
	mg2.add_option('--lumi',help=colours[5]+'Luminosity: 2320.,2320. (Single,Double)'+colours[0],default=[2320.,2320.],type='str',action='callback',callback=optsplitfloat,dest='LUMI')
...
# Selection loop
 ...
 ## Nuissance parameters
 		nVar = "CMS_vbfbb_scale_mbb_sel%s_13TeV"%S.tag
		kJES[iS] = RooRealVar(nVar,nVar,1.0)
 		nVar = "CMS_vbfbb_res_mbb_sel%s_13TeV"%S.tag
...
#
 ...
 ## File reading
	...
	#	inf[mass][styp][ftyp] = TFile.Open("flat/"+f,'read')
		inf[mass][styp][ftyp]= TFile.Open("/afs/cern.ch/work/n/nchernya/Hbb/Combine/toolkit/flat/run2/root/v21/"+f,'read')
...
### Category loop
  ...
  #### Define cut
				cut = TCut("bTagWeight*puWt*trigWt%s*(mva%s>%1.4f && mva%s<=%1.4f)"%(S.tag,S.tag,S.boundaries[C],S.tag,S.boundaries[C+1]))
  #### Draw mass
...
#### Start of RooFit part 
  ...
  ## Fit components
			#	rh[N] = RooDataHist("roohist_fit_%s"%N,"roohist_fit_%s"%N,RooArgList(x),hVBF[N])
				...
				m             = RooRealVar("mean_%s_13TeV"%Nb,"mean_%s_13TeV"%Nb,mass,mass-5,mass+5)
				s             = RooRealVar("sigma_%s_13TeV"%Nb,"sigma_%s_13TeV"%Nb,12,3,30)
