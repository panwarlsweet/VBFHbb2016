...
#
	...
	mg2.add_option('--TF',help=colours[5]+'Transfer function label: POL1 (Double)'+colours[0],default=['POL1','POL2'],type='str',action='callback',callback=optsplit)
	mg2.add_option('--bounds',help=colours[5]+'Transfer function boundaries: 60,170 (min,max)'+colours[0],default=[60.,170.],type='str',action='callback',callback=optsplitfloat,dest='X')
	mg2.add_option('--binsize',help=colours[5]+'Transfer function bin size: 5 (Double)'+colours[0],default=[5],type='str',action='callback',callback=optsplitint,dest='BINS')
...
# Setup
	...
	TF = opts.TF
	TFinfo = json.loads(filecontent("%s/vbfZbb_transfer_2015_run2_linear.json"%basepath))
...
## Load trees
	#	fin = TFile.Open("flat/Fit_data_sel%s.root"%S.tag,"read")
		fin = TFile.Open("dcap://t3se01.psi.ch:22125//pnfs/psi.ch/cms/trivcat/store/user/nchernya/VBFHbb2016/FitInputs/v25/FitZbb_BTagCSV_analysis_double_trignone_v25_trigWeightNewRegBtagCut_ZbbBdtInclFit.root","read")
...
## Category loop
		for C in range(S.ncat):
			...
			N  = "CAT%d"%(Cp)#.replace('NOM','Set A').replace('PRK','Set B') #print N
			print N
### Get histogram
...
### Fill
			CN1.cd()
			cut = "bdt_Zbb>%1.4f && bdt_Zbb<=%1.4f"%(S.boundaries[C],S.boundaries[C+1])
...
### Get ratio histogram
			...
			r = hRat["hRat_"+N]
			hStyle(r,C,False)
			r.Divide(hDat["hDat_CAT%s"%(sum([x for x in SC.ncats[0:iS]]))])#.replace('NOM','Set A').replace('PRK','Set B')
			r.SetMarkerSize(1.8)
			r.SetDirectory(0)
### Get fit function			
