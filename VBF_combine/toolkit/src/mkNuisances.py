    ...
    for fit_name, nuis_x in [('b', nuis_b), ('s',nuis_s)]:
        if nuis_x == None or sigma_p <= 0:
            ...
            if nuis_p != None:
                ...
                if fit_name == 'b':
                    pulls_b.append(valShift)
                    unc_b.append(sigShift-1)
                    nuis_names.append(name)
                if fit_name == 's':
                    pulls_s.append(valShift)
                    unc_s.append(sigShift-1)
                if options.abs:
...
if options.plotfile:
    ...
    h_pull_b = ROOT.TH1F("h_pull_b", "h_pull_b", len(pulls_b), 0, len(pulls_b))
    h_pull_s = ROOT.TH1F("h_pull_s", "h_pull_s", len(pulls_s), 0, len(pulls_s))
    h_unc_b  = ROOT.TH1F("h_unc_b",  "h_unc_b",  len(unc_b),   0, len(unc_b))
    h_unc_s  = ROOT.TH1F("h_unc_s",  "h_unc_s",  len(unc_s),   0, len(unc_s))
    k = 0
    for pull in pulls_b:
        h_pull_b.SetBinContent(k+1,pull)
        h_pull_b.GetXaxis().SetBinLabel(k+1,nuis_names[k])
        k+=1
    k = 0
    for pull in pulls_s:
        h_pull_s.SetBinContent(k+1,pull)
        h_pull_s.GetXaxis().SetBinLabel(k+1,nuis_names[k])
        k+=1
    k = 0
    for unc in unc_b:
        h_unc_b.SetBinContent(k+1,unc)
        h_unc_b.GetXaxis().SetBinLabel(k+1,nuis_names[k])
        k+=1
    k = 0
    for unc in unc_s:
        h_unc_s.SetBinContent(k+1,unc)
        h_unc_s.GetXaxis().SetBinLabel(k+1,nuis_names[k])
        k+=1
    outf = ROOT.TFile.Open(options.plotfile,"RECREATE")
    h_pull_b.Write()
    h_pull_s.Write()
    h_unc_b.Write()
    h_unc_s.Write()
