...
/*
  ...
  TFile *f2 = TFile::Open("test_Signal/run_from/mlfit.root");
...
//  w->allVars()->Print();
  ...
  //int count = 0; 
    ...
    TLegend *leg2 = new TLegend(0.70,0.61,0.94,1.-gStyle->GetPadTopMargin()-0.01);
	 leg2->SetTextFont(42);
	 leg2->SetFillStyle(-1);
    leg2->SetFillColor(0);
    leg2->SetBorderSize(0);
    leg2->SetTextFont(42);
    leg2->SetTextSize(0.038*0.98);
int increment=0;
...
//      frame2->addPlotable(hresid,"pE1");
 ...
 //   canFit->cd(1)->SetBottomMargin(0.4);
  ...
  //  frame2->GetXaxis()->SetLabelSize(0);
    ...
    if ((increment==0)||(increment==3))frame2->Draw();
...
//	 l1->SetTextSize(0.038*0.97*0.85);
    leg->AddEntry(gQCDFit,TString::Format("CAT %d",increment),"L");
    if (increment>2)leg2->AddEntry(gQCDFit,TString::Format("CAT %d",increment),"L");
...
//	}
		if (increment==2) {
    		leg->Draw(); 
	 		...
	 		canFit->SaveAs(TString::Format("%s/plot_brn/Fit_mH%s_%s_double_qcd.png",path.Data(),MASS.Data(),NAME.Data()).Data());
		}
		if (increment==6) {
    		leg2->Draw(); 
			 leg2->SetY1(leg2->GetY2()-leg2->GetNRows()*0.045*0.96);
	 		canFit->SaveAs(TString::Format("%s/plot_brn/Fit_mH%s_%s_single_qcd.png",path.Data(),MASS.Data(),NAME.Data()).Data());
		}
		increment++;
    delete ds;
  }
