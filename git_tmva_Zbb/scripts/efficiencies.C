      ...
      while ((titkey = TMVAGlob::NextKey(nextTitle,"TDirectory"))) {
         ...
         while ((hkey = TMVAGlob::NextKey(nextKey,"TH1"))) {
            ...
            if (hname.Contains( hNameRef ) && hname.BeginsWith( "MVA_" )) {
               ...
               h->Draw("csame");
					cout<<"Area under the ROC curve  =  "<< h.Integral("width")<<endl;
			//		cout<<"efficiency level for Bkg rej of 0.99568823 for mu = "<<h. <<endl;
			//		cout<<"efficiency level for Bkg rej of 0.99544052 for el = "<< <<endl;
					cout<<"Bkg rej for efficiency of 4.01E-02 for mu = "<<h->GetBinContent(h->FindBin(4.01E-02)) <<endl;
					cout<<"Bkg rej for efficiency of 4.20E-02 for el = "<<h->GetBinContent(h->FindBin(4.20E-02)) <<endl;
               hists.Add(h);
               nmva++;
            }
         }
...
void efficiencies( TString fin = "output/TMVA_main_QCD300to700_Nm1_Mqq_single.root", Int_t type = 2, Bool_t useTMVAStyle = kTRUE )
{
