...
//int main(){
void efficiency_roc_sign_Fisher(){
...
//	TString variables_names[n_variables]={ "DeltaEtaQQ", "SoftN5","CMVAV2_2" , "DeltaEtaQB",  "qgl1", "qgl2", "all"};  
	TString variables_names[n_variables]={ "DeltaEtaQQ", "SoftN5","CMVAV2_2" , "DeltaEtaBB",  "qgl1", "qgl2", "all"};  
...
//	for (int current_file=n_variables-1;current_file>=0;current_file--){
		...
		file->cd("Method_Fisher/Fisher");
		file->ls();
		if (current_file==n_variables-1){
			
			TTree *treeTrain = (TTree*)file->Get("TrainTree");
			TH1F *hist_b_signal = new TH1F("hfisher_signal","",60,-0.3,.3);	 
			TH1F *hist_b_bg = new TH1F("hfisher_bg","",60,-0.3,.3);	 
			TH1F *hist_b_purity;	 
			TProfile *hprof = new TProfile("hprof","",60,-0.3,0.3,0,200);
			float bdt;
			int index;
			float weight;
			float Mbb;
			treeTrain->SetBranchAddress("Fisher",&bdt);
...
*/
		...
		file_single->cd("Method_Fisher/Fisher");
		file_single->ls();
		if (current_file==n_variables-1){
