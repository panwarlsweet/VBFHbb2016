...
int main(int argc, char* argv[]){
	...
	int type_unc_int = atoi(argv[3]);

	const int NUNC = 9;
	TString type_unc[20];
	TString type_unc_str[20] = {"jes", "lf", "hf", "lfstats1", "lfstats2", "hfstats1", "hfstats2", "cferr1", "cferr2"};
	for (int i=0;i<NUNC;i++){
	//	type_unc[i] = "Jet_btagWeightCMVAV2_";
		type_unc[i] = type_unc_str[i];
	}
...
//		Float_t cats[10] = {-1.,0.12,0.44,0.57,0.6725};
	...
	TFile *file_nom =  TFile::Open("dcap://t3se01.psi.ch:22125//pnfs/psi.ch/cms/trivcat/store/user/nchernya/VBFHbb2016/v25_VBF_mva_JESR_full/"+type+"VBFMvaJESR_v25_weights_"+s_names[sig_num]+".root");
...
//	tree_bdt_JEnom->Draw(cmd3,cmd2);
	...
	sprintf(cmd2,"(\%sPassSelection_nom==1)*(\%strigWeight_nom*puWeight*Jet_btagWeightCMVAV2_%s_%s[\%sbtag_num1]*Jet_btagWeightCMVAV2_%s_%s[\%sbtag_num2]*\%sQGLweight)",(type.c_str()),(type.c_str()),("down"),(type_unc[type_unc_int].Data()),(type.c_str()),("down"),(type_unc[type_unc_int].Data()),(type.c_str()),(type.c_str()));	
	tree_bdt_JEnom->Draw(cmd,cmd2);
//	tree_bdt_JEnom->Draw(cmd3,cmd2);
	...
	sprintf(cmd2,"(\%sPassSelection_nom==1)*(\%strigWeight_nom*puWeight*Jet_btagWeightCMVAV2_%s_%s[\%sbtag_num1]*Jet_btagWeightCMVAV2_%s_%s[\%sbtag_num2]*\%sQGLweight)",(type.c_str()),(type.c_str()),("up"),(type_unc[type_unc_int].Data()),(type.c_str()),("down"),(type_unc[type_unc_int].Data()),(type.c_str()),(type.c_str()));	
	tree_bdt_JEnom->Draw(cmd,cmd2);
//	tree_bdt_JEnom->Draw(cmd3,cmd2);
	...
	ofstream out;
	out.open("output_txt/sysUnc_"+s_names[sig_num]+type+"bTagWeight_"+type_unc[type_unc_int]+".txt");
	out <<"Systematic uncertainties for "<<type<<"  "<<type_unc[type_unc_int]<<"  "<<s_names[sig_num]<<"  " <<endl;
	for (int i=0;i<NCATS;i++)
		out<<cats_names[i]<<"\t";
	out<<endl;	
	for (int i=0;i<NCATS;i++)
...
//	if (type_int==1)  frame2->SetMaximum(0.005);
   frame2->SetStats(0);
	...
	pave.AddText(tmp.Format("bTagWeight \%s",type_unc[type_unc_int].Data()));
	pave.AddText(tmp);
	pave.Draw("same");
...
////////
	...
	c1->Print("plots/"+s_names[sig_num]+type+"bTagWeight_"+type_unc[type_unc_int]+".png");
