...
//		Float_t cats[10] = {-1.,0.12,0.44,0.57,0.6725};
	...
	Float_t lumi = 35900.;
	const int num_ss = 12;	
	Float_t xsec[num_ss] =  {27990000,1712000,347700,32100 , 6831,1207 ,119.9,25.24, 831.76, 10.32,1461.02,3539.25};


	TString s_names[num_ss] = {"QCD_HT100to200","QCD_HT200to300","QCD_HT300to500","QCD_HT500to700","QCD_HT700to1000","QCD_HT1000to1500","QCD_HT1500to2000","QCD_HT2000toInf","TT","ST_s_inclusive","DYJetsToQQ","WJetsToQQ" };
	TString tex_s_names[num_ss] = {"VBF", "GF"};
		
 
for (int current=10;current<12;current++){
...
//	sprintf(cmd2,"(\%sPassSelection_nom==1)*(\%strigWeight_nom*puWeightDown*Jet_btagWeightCMVAV2[\%sbtag_num1]*Jet_btagWeightCMVAV2[\%sbtag_num2]*\%sQGLweight)",(type.c_str()),(type.c_str()),(type.c_str()),(type.c_str()),(type.c_str()));	
	sprintf(cmd2,"(\%sPassSelection_nom==1)*(\%strigWeightDown*puWeight*\%sQGLweight)",(type.c_str()),(type.c_str()),(type.c_str()));	
...
//	sprintf(cmd3,"\%s\%s>>\%s",(type),("Mbb"),("hist_mbb_up"));	
	sprintf(cmd2,"(\%sPassSelection_nom==1)*(\%strigWeightUp*puWeight*\%sQGLweight)",(type.c_str()),(type.c_str()),(type.c_str()));	
...
/*	for (int i=0;i<NCATS;i++)
		out<<std::fixed<<std::setprecision(5)<<Nevt_up[i]<<"\t";	
	for (int i=0;i<NCATS;i++)
		out<<std::fixed<<std::setprecision(5)<<Nevt_down[i]<<"\t";	
	for (int i=0;i<NCATS;i++)
