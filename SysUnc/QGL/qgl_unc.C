...
//	tree_bdt_JEnom->Draw(cmd3,cmd2);
	...
	hist_bdt_JEdown->Sumw2(kTRUE);
	hist_bdt_JEdown->GetXaxis()->SetTitle("BDT output"); 
	sprintf(cmd,"\%s\%s>>\%s",(type.c_str()),("VBF_bdt"),("hist_bdt_down"));	
	sprintf(cmd2,"(\%sPassSelection_nom==1)*(\%strigWeight_nom*puWeight)",(type.c_str()),(type.c_str()));	
	tree_bdt_JEnom->Draw(cmd,cmd2);
//	tree_bdt_JEnom->Draw(cmd3,cmd2);
	...
	float total_integral = hist_bdt_JEdown->Integral(0,hist_bdt_JEdown->GetNbinsX()+1);	
	cout<<total_integral<<endl;

	hist_bdt_JEnom->Scale(total_integral/hist_bdt_JEnom->Integral(0,hist_bdt_JEnom->GetNbinsX()+1));

	cout<<hist_bdt_JEnom->Integral(0,hist_bdt_JEnom->GetNbinsX()+1)<<endl;
	
	for (int i=0;i<NCATS;i++)
		SysUnc[i] = (Nevt_down[i] - Nevt_nom[i])/((Nevt_nom[i] + Nevt_down[i])/2.);


	ofstream out;
	out.open("output_txt/sysUnc_"+s_names[sig_num]+type+type_unc+".txt");
	...
	hist_bdt_JEdown->Divide(hist_bdt_JEnom);
	hist_bdt_JEdown->SetMarkerStyle(21);

 	TCanvas* c1 = new TCanvas("bdt","",800,800) ;
	c1->cd();
	TH1F *frame2 = new TH1F("frame2","",50,-1.,1.);
	frame2->SetMinimum(hist_bdt_JEdown->GetMinimum()*0.8);
	frame2->GetXaxis()->SetNdivisions(505);
   frame2->SetMaximum(hist_bdt_JEdown->GetMaximum()*1.2);
//	if (type_int==1)  frame2->SetMaximum(0.005);
   frame2->SetStats(0);
	frame2->SetXTitle(hist_bdt_JEnom->GetXaxis()->GetTitle());
	frame2->SetYTitle("Ratio to nominal");
	frame2->Draw();
...
////////
	...
	c1->Print("plots/"+s_names[sig_num]+type+type_unc+".png");
