...
int preselection_double(Int_t trigger, Int_t loopJet_max, std::vector<TLorentzVector> &jets_pv, std::vector<std::pair<int, float> > &btag_vec, std::vector<std::pair<int, float> > &pt_vec, Int_t &btag_num1, Int_t &btag_num2, Int_t &pt_num1, Int_t &pt_num2 ) {
		...
		if (jets_pv[0].Pt()<92)  not_pass=-2;
		if (jets_pv[1].Pt()<76)  not_pass=-3;
		if (jets_pv[2].Pt()<64)  not_pass=-4;
		if (jets_pv[3].Pt()<30)  not_pass=-5;
		...
		if (Mqq<240) not_pass=-6;
		if (qqDeltaEta<2.3) not_pass=-7;
		if (bbDeltaPhi>2.1) not_pass =  -8;
		if (btag_vec[0].second < 0.) not_pass = -10;
