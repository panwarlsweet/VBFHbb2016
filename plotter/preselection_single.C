...
int preselection_single(Int_t trigger, Int_t loopJet_max, vector<TLorentzVector> &jets_pv, vector<std::pair<int, float> > &blike_vec, vector<std::pair<int, float> > &pt_vec, Int_t &btag_num1, Int_t &btag_num2, Int_t &pt_num1, Int_t &pt_num2 ) {
		...
		if (Mqq<500) not_pass=-6;
		if (qqDeltaEta<4.2) not_pass=-7;
		if (bbDeltaPhi>1.6) not_pass =  -8;
