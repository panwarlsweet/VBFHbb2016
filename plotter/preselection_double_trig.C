...
int preselection_double(Int_t trigger, Int_t loopJet_max, std::vector<TLorentzVector> &jets_pv, std::vector<std::pair<int, float> > &btag_vec, std::vector<std::pair<int, float> > &pt_vec, Int_t &btag_num1, Int_t &btag_num2, Int_t &pt_num1, Int_t &pt_num2 ,Int_t &cut_count ) {
		...
		if (Mqq<240) {not_pass=-6; cut_count++;	 };
		if (qqDeltaEta<2.3){ not_pass=-7; cut_count++;	 };
		if (bbDeltaPhi>2.1){ not_pass =  -8; cut_count++;	 };
