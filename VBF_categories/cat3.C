...
//	int type_int; //double  =0; single=1
	...
	TString text_type = "Z-boson search";
	

	TString ending = "";

	TFile *file_s =  TFile::Open(path+"DYJetsToQQ_analysis_double_trignone_v25_trigWeightNewRegBtagCut_ZbbBdtIncl2.root");
	TH1F *hist_S = (TH1F*)file_s->Get("hbdt_Zbb_fine");
	hist_S->Scale(35900.);

	TFile *file_data =  TFile::Open(path+"BTagCSV_analysis_double_trignone_v25_trigWeightNewRegBtagCut_ZbbBdtIncl2.root");
	TH1F *hist_D = (TH1F*)file_data->Get("hbdt_Zbb_fine");
...
max=0;
	...
	ofstream out;
	out.open("output_txt/3categories_splusb"+binning+".txt");
	out<<"max all, sqrt : "<<max_all<<"  , "<<sqrt(max_all)<<endl;
	out<<"borders of categories : "<<START<<"   "<<border1<<"   "<<border2<<" , END = "<< END <<endl;
	out<<"S**2/S+B in each category : "<<max1_final<<"   "<<max2_final<<"   " << max3_final<< "    , max = "<<max<<endl;
	out<<"sqrt(S**2/S+B) in each category : "<<sqrt(max1_final)<<"   "<<sqrt(max2_final)<<"   " << sqrt(max3_final)<< "    , max = "<<sqrt(max)<<endl;
	out.close();
	cout<<"max all, sqrt : "<<max_all<<"  , "<<sqrt(max_all)<<endl;
	cout<<"borders of categories : "<<START<<"   "<<border1<<"   "<<border2<<" , END = "<< END <<endl;
	cout<<"S**2/S+B in each category : "<<max1_final<<"   "<<max2_final<<"   " << max3_final<< "    , max = "<<max<<endl;
	cout<<"sqrt(S**2/S+B) in each category : "<<sqrt(max1_final)<<"   "<<sqrt(max2_final)<<"   " << sqrt(max3_final)<< "    , max = "<<sqrt(max)<<endl;
...
//	frame2->SetMaximum(hist_B->GetMaximum()*1.2);
	...
	line2.Draw("same");
	gPad->RedrawAxis();
	c1->Print("plots/BDT_output_signal_bg_cat3.png");
