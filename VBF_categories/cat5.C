...
// CMS info
	...
	int i=0; int j=0; int k=0; int l=0; int m=0; int p=0; int ll=0;
	double bin=0.;
...
max=0;
		...
		//	cout<<max2<<endl;
			do{
				...
				do{	
					...
					b5=0.;
					do  {
						s5+=hist_S->GetBinContent(ll+1);
						b5+=hist_B->GetBinContent(ll+1);
						bin=(double) hist_S->GetBinCenter(ll+1+1);
						ll++;
					} while (bin < END);
					if ((b5+s5)!=0 )max5=pow(s5,2)/(b5+s5);
	...
	ofstream out;
	out.open("output_txt/5categories_splusb"+text_type+binning+".txt");
	out<<"max all, sqrt : "<<max_all<<"  , "<<sqrt(max_all)<<endl;
	out<<"borders of categories : "<<START<<"   "<<border1<<"   "<<border2<<"   "<<border3<<"   "<<border4<<" , END = "<< END <<endl;
	out<<"S**2/S+B in each category : "<<max1_final<<"   "<<max2_final<<"   " << max3_final<<"  "<<max4_final<<"  "<<max5_final<< "    , max = "<<max<<endl;
	out<<"sqrt(S**2/S+B) in each category : "<<sqrt(max1_final)<<"   "<<sqrt(max2_final)<<"   " << sqrt(max3_final)<<"   " << sqrt(max4_final) <<"   " << sqrt(max5_final)<<"    , max = "<<sqrt(max)<<endl;
	cout<<"max all, sqrt : "<<max_all<<"  , "<<sqrt(max_all)<<endl;
	cout<<"borders of categories : "<<START<<"   "<<border1<<"   "<<border2<<"   "<<border3<<"   "<<border4<<" , END = "<< END <<endl;
	cout<<"S**2/S+B in each category : "<<max1_final<<"   "<<max2_final<<"   " << max3_final<<"  "<<max4_final<<"  "<<max5_final<< "    , max = "<<max<<endl;
	cout<<"sqrt(S**2/S+B) in each category : "<<sqrt(max1_final)<<"   "<<sqrt(max2_final)<<"   " << sqrt(max3_final)<<"   " << sqrt(max4_final)<<"   " << sqrt(max5_final) <<"    , max = "<<sqrt(max)<<endl;
	out.close();
...
//	frame2->GetYaxis()->SetRangeUser(80,150);
   frame2->SetStats(0);
	...
	line4.Draw("same");
	gPad->RedrawAxis();
	c1->Print("plots/VBF_BDT_output_signal_bg_"+text_type+"cat5.png");
