...
int set_type=atoi(argv[1]); // double, single (0/1)
int region_type=atoi(argv[2]); // 0 - analysis, 1 - control DY , control  top
 
const int nfiles  = 10; //9;  //11;
TString leg_names[nfiles] = {"Data","Z(ll) + jets","W(l#nu) + jets","Z + jets","W + jets","Single Top","t#bar{t}","QCD","GF, m(H) = 125 GeV","VBF, m(H) = 125 GeV"};
TString leg_names_signals[nfiles] = {"VBF, m(H) = 125 GeV","GF, m(H) = 125 GeV"};
TString set_names[2] = {"DoubleB","SingleB"}; 


TString file_names[nfiles] = {"BTagCSV","DYJetstoLL_amc","WJetsToLNu","DYJetsToQQ","WJetsToQQ","ST","TT","QCD","GluGluHToBB_M-125","VBFHToBB_M-125"};
TString file_names_JESup[nfiles] = {"BTagCSV","DYJetstoLL_amc","WJetsToLNu","DYJetsToQQ","WJetsToQQ","ST","TT","QCD","GluGluHToBB_M-125","VBFHToBB_M-125"};
TString file_names_JESdown[nfiles] = {"BTagCSV","DYJetstoLL_amc","WJetsToLNu","DYJetsToQQ","WJetsToQQ","ST","TT","QCD","GluGluHToBB_M-125","VBFHToBB_M-125"};
int bg_begin;
if (region_type==0) bg_begin=3;
else bg_begin=1;

int FILLCOLOR[nfiles] = {1,kYellow-4,kYellow-8,kYellow-9,kOrange-9,kYellow-10, kOrange-2,kRed-9,kBlue,kRed};
//int LINECOLOR[nfiles] = {1,kYellow-9,kOrange-9,kYellow-10, kOrange-2,kRed-9,kRed};
int LINECOLOR[nfiles] = {1,1,1,1,1,1,1,1,kBlue,kRed};
int LINESTYLE[nfiles] = {1,1,1,1,1,1,1,1,2,1};
...
TString region[3]={"_analysis","_controlDY","_controlTop"}; 
...
for (int i=0;i<nfiles;i++){
   file_names[i].Prepend("dcap://t3se01.psi.ch:22125//pnfs/psi.ch/cms/trivcat//store/user/nchernya/VBFHbb2016/plotterOutput/v25/");
	...
	if (i==0) 	file_names[i].Append("_trignone_v25_VBF_newReg_JES2nom");
	if (i!=0) {
		file_names[i].Append("_trignone_v25_VBF_newReg_JES2nom");
		file_names_JESup[i].Append("_trignone_v25_VBF_newReg_JES2up");
		file_names_JESdown[i].Append("_trignone_v25_VBF_newReg_JES2down");
	}
//	file_names[i].Append("_trignone_v25_notriggercorr");
	file_names_JESup[i].Append(".root");
	file_names_JESdown[i].Append(".root");
	file_names[i].Append(".root");
}
TString trigger[2] = {"", ""};
//TString dir_name= "plots_v25_trigWeightNewRegBtagCut_noQCD";
//TString dir_name= "Zbb/plots_v25_trigWeightNewRegBtagCutIncl";
TString dir_name="JES/plots_v25_JES";
//TString dir_name="notriggers/plots_v25_notriggers";
...
files=0;
do{
	if ((region_type==0) && ((files==1)||(files==2))){
		files++;
		continue;
	}
	string tmp_file_name = file_names[files].Data();
	if ((region_type!=0) && (tmp_file_name.find("ToQQ")!=std::string::npos)){
		files++;
		continue;
	}
	...
	if (files!=0) {
		...
		if (region_type!=0) {
			if (file_name_tag.find("TT")!=std::string::npos)  QCDint= histos_mc[hist]->Integral(0,histos_mc[hist]->GetNbinsX()+1);
			else if (file_name_tag.find("QCD")==std::string::npos) MCint+=histos_mc[hist]->Integral(0,histos_mc[hist]->GetNbinsX()+1);
			//else  MCint+=histos_mc[hist]->Integral(0,histos_mc[hist]->GetNbinsX()+1);
		} 
	}
	files++;	
...
//if (region_type!=0)ratio[counter_ratio] = 1.;
counter_ratio++;
...
do{
	...
	if ((region_type!=0)&&(file_name_tag.find("QCD")!=std::string::npos)){
		files++;
		continue;
	}
	TFile *file_initial;
	...
	for (int hist=0;hist<nhistos;++hist){
		...
		if (region_type!=0) ((TH1F*)file_initial->Get(hist_names[hist]))->Rebin(divisor);
//		if (hist_names[hist].CompareTo("hblike1")==0) ((TH1F*)file_initial->Get(hist_names[hist]))->Rebin(4);
//		if (hist_names[hist].CompareTo("hblike2")==0) ((TH1F*)file_initial->Get(hist_names[hist]))->Rebin(4);
//		if (hist_names[hist].CompareTo("hbtag")==0) ((TH1F*)file_initial->Get(hist_names[hist]))->Rebin(4);
//		if (hist_names[hist].CompareTo("hbtag2")==0) ((TH1F*)file_initial->Get(hist_names[hist]))->Rebin(4);
		if (hist_names[hist].CompareTo("hbdt_Zbb")==0) ((TH1F*)file_initial->Get(hist_names[hist]))->Rebin(4);
		...
		if (region_type==0) if (file_name_tag.find("QCD")!=std::string::npos)  histos[hist]->Scale(ratio[0]);  
		if (region_type!=0) if (file_name_tag.find("TT")!=std::string::npos)  histos[hist]->Scale(ratio[0]);  
...
// Signal now is included in the sum total MC
		...
		// Signal is not included in stack
	 	if ((files>=bg_begin)&&(files<(nfiles-2))) stacks[hist]->Add(histos[hist]);
		...
		if (files>=bg_begin) totalMC+=histos[0]->Integral(0,histos[0]->GetNbinsX()+1);
		...
		if (region_type==0) if (files==nfiles-1) out_efficiency<<"QCD MC scaled with "<<"\t \t \t  "<<std::setprecision(13)<<ratio[0]<<endl;
		if (region_type!=0) if (files==nfiles-1) out_efficiency<<"TT MC scaled with "<<"\t \t \t  "<<std::setprecision(13)<<ratio[0]<<endl;
files++;
}while (files<nfiles);
...
do{
	...
	for (int hist=0;hist<nhistos;++hist){
		if (hist_names[hist].CompareTo("hbdt_Zbb")==0) ((TH1F*)file_initial_up->Get(hist_names[hist]))->Rebin(4);
		histos_JESup[hist] = (TH1F*)file_initial_up->Get(hist_names[hist])->Clone("hup");
		histos_JESup[hist]->Scale(lumi);  //top
		if (file_name_tag.find("QCD")!=std::string::npos)  histos_JESup[hist]->Scale(ratio[0]);  
	...
	for (int hist=0;hist<nhistos;++hist){
		if (hist_names[hist].CompareTo("hbdt_Zbb")==0) ((TH1F*)file_initial_down->Get(hist_names[hist]))->Rebin(4);
		histos_JESdown[hist] = (TH1F*)file_initial_down->Get(hist_names[hist])->Clone("hdown");
		histos_JESdown[hist]->Scale(lumi);  //top
		if (file_name_tag.find("QCD")!=std::string::npos)  histos_JESdown[hist]->Scale(ratio[0]);  
...
if (region_type==0) leg->AddEntry(histos_for_legened[order_legend[nfiles-1]],leg_names_signals[0],"L");
if (region_type==0) leg->AddEntry(histos_for_legened[order_legend[nfiles-2]],leg_names_signals[1],"L");
for (int i=bg_begin;i<nfiles-4;i++){
	if ((region_type!=0) && ((i==3)||(i==4))) continue;
	leg->AddEntry(histos_for_legened[order_legend[i]],leg_names[i],"F");
}
TString tmp_str;
if (region_type==0) {
	leg->AddEntry(histos_for_legened[nfiles-4],leg_names[nfiles-4],"F");
	leg->AddEntry(histos_for_legened[nfiles-3],tmp_str.Format("QCD (#times %2.2f)",ratio[0]),"F");
}
else {
	leg->AddEntry(histos_for_legened[nfiles-4],tmp_str.Format("t#bar{t} (#times %2.2f)",ratio[0]),"F");
//	leg->AddEntry(histos_for_legened[nfiles-3],leg_names[nfiles-3],"F"); //comment out of no QCD added
}
...
//	for (int i=0;i<4;i++){
	...
	//	tex_k->Draw();
//		tex_set->Draw();
		if ((d_value>0.1)&&(d_value<1.)&&(region_type==0)&&(hist_names[i].CompareTo("hbdt_Zbb")!=0)) disc_value_text->Draw();
    	stacks[i]->Draw("same");	
...
///////////////////////////////////////////////////
	...
	//	leg2->Draw("same");
      ...
      frame2->SetTitleSize(0.13, "XYZ");
		...
		if (hist_names[i].CompareTo("hbdt_Zbb")==0) frame2->SetXTitle("Z-boson BDT output");
