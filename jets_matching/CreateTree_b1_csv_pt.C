   ...
   for (Long64_t jentry=0; jentry<nentries;jentry++) {
	   ...
	   JetList jetList_CSV, jetList_pt, jetList_eta; 
		...
		for (int i=0;i<nJet;i++){
			...
			jets_btagCMVA.push_back(Jet_btagCMVA[i]);
			good_jets++;
		}
		...
		for (int i=0;i<loopJet_max;i++){
			if (jets_btagCMVA[i] > max_btag){
				max_btag=jets_btagCMVA[i];
				max_btag_idx=i;
			}
		}	
		btag_num1=max_btag_idx;
		Bjet1.SetPtEtaPhiM(jets_pv[btag_num1].Pt(),jets_pv[btag_num1].Eta(),jets_pv[btag_num1].Phi(),jets_pv[btag_num1].M());	
		max_btag=-20;
		max_btag_idx=0;
		for (int i=0;i<loopJet_max;i++){
			if ((jets_btagCMVA[i] > max_btag) && (i!=btag_num1)){
				max_btag=jets_btagCMVA[i];
				max_btag_idx=i;
			}
		}	
		btag_num2=max_btag_idx;
		Bjet2.SetPtEtaPhiM(jets_pv[btag_num2].Pt(),jets_pv[btag_num2].Eta(),jets_pv[btag_num2].Phi(),jets_pv[btag_num2].M());	
		
		int qjet_count=0;
		for (int i=0;i<loopJet_max;i++){
			if ((i!=btag_num1) && (jets_indices[i]!=btag_num2)){
				if (qjet_count==0) pt_num1=i;
				if (qjet_count==1) pt_num2=i;
				qjet_count++;
			}	
	...
	ofstream out("VBFHbb_jets_matching_"+region+"_dr08.txt"); 
